...
//add an endpoint app.get('/checkups' that parses the query to find the field to sort by sort_by. Implement sorting by date and by pet_id. The default for sorting should be by id which is the natural default.
app.get('/checkups', (req, res) => {
  db.all('SELECT * FROM checkups ORDER BY ' + (req.query.sort_by || 'id'), (err, items) => {
    if (err) {
      res.status(500).json({'error': err.message});
      return;
    }
...
//     if (sort_by == 'date'){
//       query = 'SELECT * FROM checkups ORDER BY date ASC';
//     }
//     else if (sort_by == 'pet_id'){
//       query = 'SELECT * FROM checkups ORDER BY pet_id ASC';
//     }
//     else {
