...
app.post('/pets/:petId/checkups', (request, response) => {
    console.log(request.body);
    db.get(`INSERT INTO checkups (date, pet_id, checkup_result) VALUES (?, ?, ?) RETURNING id`,
     [request.body.date, request.body.pet_id, request.body.checkup_result],
        (err, result) => {
            if (err) {
                response.status(500).json({ 'error': err.message });
                return;
